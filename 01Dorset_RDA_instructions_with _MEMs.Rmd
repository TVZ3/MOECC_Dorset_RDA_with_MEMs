...
### Droping columns with too many NAs (that would make the data set too small) and
### some chemistry variables not important to the algal community
```{r results="hide"}
names(dorset)
...
=======
### Subsetting algae data by total biomass versus relative abundance
>>>>>>> ba21f295ec5a0a017bbe589ee2c6228b91115f67
```{r}
...
### RDA Creation
#### RDA of Hellinger-transformated species data constrained by non-collinear predictor variables that 
#### came out as important in the above forward selection procedure
```{r}
par(mfrow=c(1,1))
...
##### E.G. We see below that "e" is the overlap between X1 (morphology) and X2 (Chemistry). We can see
##### above that "e" explains 7.95% (0.0795)  of the variation in the model
```{r}
showvarparts(4)
```